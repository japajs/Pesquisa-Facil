...
import { AtaPDF } from "@/lib/relatorios/pdf-ata"
...
export async function GET(
  ...
  if (assembleia.status !== "encerrada") {
    return new NextResponse("A ata só pode ser gerada depois que a assembleia for encerrada.", {
      status: 400,
    })
  }
  ...
  const element = React.createElement(AtaPDF, {
    assembleia,
    condominio,
    apuracao,
    votosDetalhados,
  ...
  const filename = `${APP_NAME}_Ata_${condo}_${titulo}.pdf`
